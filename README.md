...
| ⚖️ **สรุปความสมดุลภาระงาน (Fairness)** | คำนวณคะแนนภาระงานต่อคน (ถ่วงน้ำหนักตามความหนักของกะ + โบนัสเวรควบ) เพื่อดูความสมดุลของการจัดเวร |
...
│   ├── auto-schedule.jsx       # จัดเวรอัตโนมัติ (engine: utils/scheduleEngine/)
│   ├── reservation.jsx         # จองเวรล่วงหน้า (ปฏิทินเลือกกะ)
│   ├── fairness.jsx            # สรุปความสมดุลภาระงาน
...
engine ใหม่อยู่ที่ `utils/scheduleEngine/` (pure function, deterministic) — **กติกาทั้งหมดสกัดจากข้อมูลการขึ้นเวรจริง 7,473 รายการ** หลักฐานและรายละเอียดอยู่ใน [`AUTO_SCHEDULE.md`](AUTO_SCHEDULE.md)

ขั้นตอน: `POST /api/auto-schedule` โหลดพนักงานของแผนก + เวร/วันลา/อบรมที่กรอกไว้ (= ช่องห้ามแตะ) + เวร 7 วันสุดท้ายของเดือนก่อน (บริบทต่อเนื่องข้ามเดือน) → engine เติมช่องว่างวันต่อวัน (ดึก → บ่าย → เช้า → เช้าOT → บ่ายOT) โดยเลือกคนตามคะแนน transition จริง + ความสมดุล → พรีวิวใน `AutoSchedulePanel` → `POST /api/auto-schedule/apply` บันทึก (โหมดเติมช่องว่าง / ล้างเวรทำงานแล้วแทนที่ โดยคงวันลา-อบรมไว้)

**Hard rules** (จากคู่ที่ไม่เคยปรากฏในข้อมูลจริง): มีบ่ายเมื่อวานห้ามดึกวันนี้ (รวมรอยต่อเดือน), เวรควบเฉพาะ ชบ/ดบ, ดึกติดกัน ≤3 คืน, หัวหน้าหอขึ้นเฉพาะเช้า (ส–อา เป็น OT) — ลูปหมุนจริง **ช→ด→ดบ→ชบ→หยุด** ใช้เป็นคะแนน ไม่ใช่กฎบังคับ
...
| `/profile` | โปรไฟล์ |
| `/auto-schedule` | จัดเวรอัตโนมัติ (แอดมิน — จัดคนเข้าแผนกทำที่หน้าตารางเวร ปุ่ม "จัดคนขึ้นเวร") |
| `/reservation` | จองเวรล่วงหน้า (ค่าเริ่มต้น = เดือนหน้า) |
| `/fairness` | สรุปความสมดุลภาระงานต่อคน |
