...
export const TableSelectMonth = ({
  ...
  const [{ data: userList, loading: userListLoading, error: userListError }, getUserDropdown] =
    useAxios({ url: `/api/user?month=${monthValue}&year=${yearValue}`, method: "GET" });

  const [{ data: shif, loading: shifLoading, error: shifError }] = useAxios({
    url: "/api/shif",
  });

  const [{ data: location, loading: locationLoading, error: locationError }] = useAxios({
    url: "/api/location",
  });
  ...
  const [{ loading: dutyUserLoading, error: dutyUserError }, executeUserDuty] = useAxios(
  ...
  if (userError || shifError || dutyError || dutyDeleteError || locationError || userListError || dutyUserError)
  ...
  return (
    <>
      ...
      <div className="w-100 bg-white shadow-xl p-5 m-10 rounded-md overflow-x-auto">
        {userLoading || shifLoading || dutyLoading || dutyDeleteLoading || locationLoading || userListLoading || dutyUserLoading ? (
          <LoadingComponent />
        ) : (
          <></>
        )}
            ...
            {user?.map((person, key) => {
              ...
              return (
                <tr key={key} className="border odd:bg-green-100">
                  ...
                  <td className="border">{person.Duty.find(x => x.Location?.name)?.Location?.name}</td>
      ...
      <div className="w-100 bg-white shadow-xl p-5 m-10 rounded-md overflow-x-auto">
        <div className="justify-center text-center h2 text-xl font-normal leading-normal mt-0 mb-2 text-black">จัดคนขึ้นเวร</div>
        <form class="w-full" onSubmit={async (x) => {
          x.preventDefault();
          ...
          <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full md:w-2/5 px-3 mb-6 md:mb-0">
              ...
              <div class="relative">
                <select id="userId" name="userId" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
                  {userList?.filter(x => !user?.map(x => { return x.id }).includes(x.id))?.map((person, index) => (
            ...
            <div class="w-full md:w-2/5 px-3 mb-6 md:mb-0">
              ...
              <div class="relative">
                <select id="locationId" name="locationId" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-state">
            ...
            <div class="w-full md:w-1/5 px-3 mb-6 md:mb-0">
              ...
              <div class="relative">
                <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">บันทึก</button>
              </div>
            </div>
          </div>
        </form>
